...
**Q：Checkpoint 能不能解决重复扣款、重复发邮件这类副作用问题？**

不能。Checkpoint 只能恢复图状态，不能自动回滚外部世界。写操作节点要用幂等键、pending action、外部状态查询和补偿策略；恢复时先查业务系统是否已执行，再决定跳过、补偿或转人工。完整生产化答法见 [LangGraph 生产化与系统设计](../engineering/langgraph-production)。
