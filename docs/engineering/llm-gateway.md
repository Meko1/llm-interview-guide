...
模型网关常被追问“怎么省钱、怎么选模型、怎么证明降级不影响效果”。可以按三层回答：

1. **请求层治理**：虚拟 Key、RPM/TPM、预扣 token、超时重试、熔断、failover。
2. **路由层治理**：简单任务走小模型，复杂任务升级强模型；敏感租户固定私有化通道；主模型 429 或超时进入备用链。
3. **平台层治理**：路由策略、prompt、模型版本进入评测门禁；每次变更要看质量、成本、延迟和安全指标。

| 追问 | 推荐回答 |
| --- | --- |
| 怎么防止成本失控？ | 租户预算、RPM/TPM 双限流、语义/精确缓存、简单任务小模型、异常成本告警和熔断 |
| 怎么判断该走大模型还是小模型？ | 先用规则按任务类型和输入长度分层，再用低价模型做难度判断或级联升级 |
| fallback 会不会导致效果变差？ | 会，所以备用模型的 prompt、工具调用、JSON 输出和安全拒答都要进入评测集 |
| 模型升级谁来拍板？ | 不能只由业务代码决定，应进入 MaaS 的模型目录、评测门禁、灰度和回滚流程 |

更完整的平台化答案见 [MaaS 平台与模型服务治理](/engineering/maas-platform)；微调、DPO、模型平台的面试速答见 [微调与模型平台高频问答](/interview/finetuning-platform-qna)。

## 面试专项：预算、配额、缓存与降级闭环

网关层负责把平台策略落实到每一次请求。一个可上线的成本闭环可以这样讲：

1. **请求进入前**：校验虚拟 Key、租户预算、RPM/TPM、模型权限；输出 token 未知时按 `max_tokens` 或历史分位数预扣。
2. **路由时**：按任务类型、上下文长度、租户等级、合规标签和当前健康状态选模型；简单任务优先小模型，高风险任务固定强模型或私有化通道。
3. **生成中**：流式请求持续统计 token，超预算可截断、降级或转排队；上游 429/5xx 按错误类型决定重试、fallback 或失败。
4. **返回后**：校正实际 token、写成本明细、记录缓存命中、路由原因、降级原因和 trace。
5. **复盘时**：用报表找高成本应用、高价模型误用、缓存失效、prompt 变长和异常重试。

这段和 MaaS 的区别是：MaaS 定规则、审批和报表；网关执行规则、计量和请求级止血。
