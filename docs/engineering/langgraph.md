...
## 面试专项：LangGraph 状态设计模式

LangGraph 面试容易从“State 是什么”追问到“State 怎么设计才不会乱”。可按 **持久化字段、临时字段、控制字段、审计字段** 四类拆：

| 字段类型 | 示例 | 设计原则 |
| --- | --- | --- |
| 持久化业务状态 | `task_id`、`user_id`、`tenant_id`、证券代码、订单号、引用文档 ID | 用于恢复和审计，字段稳定、可序列化、少存敏感明文 |
| 临时运行态 | 当前候选答案、临时检索结果、排序分数 | 可重算的不长期存，避免 checkpoint 膨胀 |
| 控制字段 | `step_count`、`retry_count`、`next_action`、`last_tool`、`risk_level` | 用来防死循环、走条件边、触发降级和人审 |
| 审计字段 | `model_version`、`prompt_version`、`tool_call_id`、`approval_status` | 线上回放和合规必备，和 trace 对齐 |

### Checkpoint 恢复、幂等与重复执行

Checkpoint 恢复最怕“状态恢复了，但外部副作用已经发生”。写操作节点必须按后端事务思维设计：

- 每次写操作生成 `idempotency_key`，同一 key 重试只返回第一次结果。
- 工具节点先写 pending action，确认后执行真实副作用。
- checkpoint 里记录 `tool_call_id`、参数摘要、执行状态、外部业务单号。
- resume 时先查询外部状态，再决定继续、跳过、补偿或转人工。
- 发邮件、扣款、建单、删除这类副作用节点默认串行，不做并行 tool calls。

### Trace 如何用于线上排障

一条可用 trace 至少包含：用户/租户、入口请求、每个节点的输入输出摘要、模型和 prompt 版本、工具名和参数摘要、工具耗时和错误码、retrieved_docs、条件边选择原因、checkpoint id、最终状态。这样才能判断问题是规划错、检索错、工具参数错、权限错、模型幻觉，还是条件边设计错。

面试可复述：

> 我会把 State 设计成可恢复、可审计、可控流的结构，而不是把所有上下文都塞进去。可重算的大结果只存引用，控制字段用于限步和重试，写操作用幂等键和 pending 状态避免恢复后重复执行。线上排障时按 trace 看每个节点、边和工具调用，定位失败发生在哪一层。
