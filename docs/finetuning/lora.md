...
## 九、面试专项：LoRA/QLoRA 参数怎么调

面试官追问“你 LoRA 参数怎么定”，不要只背 `r=16, alpha=32`，要讲清 **任务复杂度、数据量、显存、过拟合风险、部署方式** 的平衡。

| 参数 | 影响 | 起步建议 | 追问答法 |
| --- | --- | --- | --- |
| `r` | 旁路容量、可训练参数量 | 简单格式/分类 8-16；复杂生成 16-64 | r 不一定越大越好，过大可能过拟合；先看验证集和通用能力回归 |
| `alpha` | LoRA 更新强度 | 常取 r 的 1-2 倍 | alpha 太大可能让旁路影响过强，输出风格漂移；太小学习不足 |
| `target_modules` | LoRA 加在哪些层 | 从 `q_proj/v_proj` 起步，复杂任务扩到 `k/o_proj` 和 FFN | 覆盖更多关键层通常比盲目增大 r 更有效 |
| `dropout` | 防过拟合 | 0-0.1，常用 0.05 | 小数据集加 dropout，大数据/稳定任务可降低 |
| 学习率 | 收敛速度和稳定性 | 1e-4 到 3e-4 常见 | loss 震荡或遗忘明显先降 lr；收敛太慢再小幅升 |
| batch / gradient accumulation | 梯度稳定性、显存 | 显存不够用梯度累积 | 有效 batch 太小会不稳，太大可能泛化差 |
| sequence length | 长样本覆盖和显存 | 按真实场景截断分布设定 | 训练长度低于线上输入长度，长回答和多轮对话容易退化 |
| 量化方式 | 显存与训练速度 | 显存紧张用 QLoRA NF4 | QLoRA 量化冻结基座，LoRA 旁路高精度训练，速度略慢但省显存 |

### 一个可复述的调参流程

1. 先用 prompt-only 或 RAG-only 建 baseline，确认微调确实解决的是格式/风格/任务行为。
2. 从 `r=16, alpha=32, dropout=0.05, target=attention+FFN` 起步，资源紧张用 QLoRA。
3. 用独立验证集看任务准确率、格式遵循率、通用能力回归和安全拒答。
4. 如果欠拟合，优先扩大 target modules 或增加数据质量，再考虑提高 r。
5. 如果过拟合，降低 r/lr/epoch，增加 dropout，混入通用 replay 数据。
6. 上线前决定是否 merge LoRA；多租户多任务场景保留 adapter 热插拔，单一稳定任务可合并权重。

**一句话总结：** LoRA 调参不是“把 r 调大”，而是在有限显存下找到足够容量、不过拟合、可部署、可回滚的适配器配置。
