...
## 八、面试专项：RLHF/DPO 项目怎么讲

如果项目里做过“模型更符合业务偏好”，建议按 **偏好数据 -> 方法选型 -> 训练约束 -> 评估门禁 -> 上线监控** 来讲，而不是只说“我跑了 DPO”。
...
线上问题 / 专家题库 / bad case
  -> 多模型或多 prompt 生成候选回答
  -> 人工/专家/强模型按 rubric 做 pairwise 标注
  -> 清洗冲突样本、去重、分难度分场景
  -> 划分 train / dev / held-out test
  -> DPO / PPO / KTO 等偏好优化
  -> pairwise judge + 人工抽检 + 安全集回归
```

rubric 至少覆盖：帮助性、事实性、简洁性、格式遵循、安全拒答、引用忠实度。高风险场景要把“该拒绝”和“不该拒绝”的样本都放进去，否则很容易训练出过度拒答。

### 方法选型

| 场景 | 推荐 | 理由 |
| --- | --- | --- |
| 有 chosen/rejected 偏好对，资源有限 | DPO | 不训 RM，不跑 PPO，稳定省资源 |
| 只有点赞/点踩这类单点反馈 | KTO | 不要求同一 prompt 下成对比较 |
| 想把 SFT 与偏好优化合并 | ORPO | 单阶段、少一份参考模型 |
| 数学/代码/推理有自动验证奖励 | GRPO / RLVR | 在线探索上限更高 |
| 安全策略复杂、需要在线探索 | PPO / RLHF | 成本高，但可利用 RM 在线优化 |

### 评估怎么做

- **偏好胜率**：新模型相对旧模型的 pairwise win rate。
- **帮助性**：是否真正解决用户问题，不只是更长、更礼貌。
- **事实性/忠实度**：尤其是 RAG 场景，回答是否严格基于检索材料。
- **拒答边界**：危险请求拒答率、正常请求误拒率都要看。
- **格式遵循**：JSON、表格、引用编号、业务话术是否稳定。
- **通用能力回归**：避免对齐后数学、代码、通用问答明显下降。

### 面试可复述版本

> 我们先用 SFT 得到业务可用的初始模型，但发现它在简洁性、拒答边界和事实忠实度上不稳定。于是从线上 bad case 和专家题库构造偏好对，用 rubric 标注 chosen/rejected。方法上优先选 DPO，因为它不需要额外训练奖励模型，也不用 PPO 的四模型链路，工程稳定性更好。上线前用 pairwise judge、人工抽检、安全红队集和通用能力回归做门禁；线上继续看误拒率、投诉率、人工转接率和 bad case 回流，防止 reward hacking 和过度拒答。
