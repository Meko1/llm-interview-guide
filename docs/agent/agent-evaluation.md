...
- **权限最小化 + 沙箱**：工具按需授权，写操作隔离、可回滚、可审计；工具执行边界的系统设计见 [Agent 工具安全与权限边界](/agent/tool-safety)。
...
Agent 安全比普通聊天机器人更复杂，因为它不只生成文本，还可能检索私有数据、调用工具、写业务系统。面试可以用这张矩阵回答；如果追问「工具权限怎么画、写操作怎么确认」，转到 [Agent 工具安全与权限边界](/agent/tool-safety) 继续展开。
