...
| 方案 | 放在哪一段 | 解决什么 | 代价 |
| --- | --- | --- | --- |
| Hybrid Search | 检索中 | 专有名词、编号、精确匹配召回差 | 需要融合排序和两套索引 |
| Multi-Query | 检索前 | 用户问法单一导致召回覆盖不足 | 多次检索，延迟和成本上升 |
| HyDE | 检索前 | 问题和文档表述差异大 | 额外 LLM 调用，假想答案可能偏 |
| Rerank | 检索后 | Top-K 噪声多、排序不准 | 增加精排延迟 |
| Parent-child Chunking | 切分/组装 | 小块检索准但上下文不完整 | 索引和上下文映射更复杂 |
| Contextual Compression | 检索后 | 上下文太长、噪声多 | 压缩器可能删掉关键信息 |
| GraphRAG | 高级索引 | 跨文档、多跳、全局总结 | 构建成本高、更新复杂 |

选型口诀：**召回漏就扩 query 或混合检索，排序乱就 rerank，上下文碎就父子分块，跨文档关系强再考虑 GraphRAG**。
